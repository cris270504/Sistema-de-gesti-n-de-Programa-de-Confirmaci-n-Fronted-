-- =====================================================================
-- Cómo se llama a la persona del programa (antes fijo por tipo:
-- Confirmando/Comulgante/Participante) ahora es editable por el admin de
-- cada parroquia y por el proveedor -- mismo mecanismo que programa_tipo:
-- vacío = cae al default automático según programa_tipo, con texto = lo
-- usa tal cual.
-- =====================================================================

alter table public.parroquia_configuraciones
    add column if not exists persona_nombre_singular text,
    add column if not exists persona_nombre_plural text;

alter table public.parroquia_configuraciones
    drop constraint if exists parroquia_config_persona_singular_len_check;
alter table public.parroquia_configuraciones
    add constraint parroquia_config_persona_singular_len_check
    check (persona_nombre_singular is null or length(persona_nombre_singular) <= 40);

alter table public.parroquia_configuraciones
    drop constraint if exists parroquia_config_persona_plural_len_check;
alter table public.parroquia_configuraciones
    add constraint parroquia_config_persona_plural_len_check
    check (persona_nombre_plural is null or length(persona_nombre_plural) <= 40);
...
DECLARE
    ...
    _persona_s  text   := nullif(trim(coalesce(p_config->>'persona_nombre_singular', '')), '');
    _persona_p  text   := nullif(trim(coalesce(p_config->>'persona_nombre_plural', '')), '');
    _dias       int;
    _tipos      jsonb;
    _proc       jsonb;
    _roles      jsonb;
...
BEGIN
    ...
    IF _persona_s IS NOT NULL AND length(_persona_s) > 40 THEN
        RAISE EXCEPTION 'El nombre en singular admite hasta 40 caracteres' USING ERRCODE = 'check_violation';
    END IF;
    IF _persona_p IS NOT NULL AND length(_persona_p) > 40 THEN
        RAISE EXCEPTION 'El nombre en plural admite hasta 40 caracteres' USING ERRCODE = 'check_violation';
    END IF;
    ...
    ON CONFLICT (parroquia_id) DO UPDATE SET
        ...
        persona_nombre_singular     = EXCLUDED.persona_nombre_singular,
        persona_nombre_plural       = EXCLUDED.persona_nombre_plural,
    ...
    RETURN jsonb_build_object(
        ...
        'configuracion', jsonb_build_object(
            ...
            'persona_nombre_singular',    _row->>'persona_nombre_singular',
            'persona_nombre_plural',      _row->>'persona_nombre_plural',
