...
export const useRequisitosStore = defineStore('requisitos', {
  ...
  actions: {
    ...
    async fetchById(id) {
      const existingRequisito = this.byId(id);
      if (existingRequisito) return existingRequisito;

      this.loading = true
      this.error = null
      try {
        const requisitoId = Number(id);
        const requisito = await getRequisitoById(requisitoId)
        const idx = this.items.findIndex(s => s.id === requisitoId)
    ...
    async add(RequisitoPayload) {
      try {
        const response = await createRequisito(RequisitoPayload);
    ...
    async remove(id, nombre) {
      ...
      const ok = await confirmarEliminacion(nombre || `requisito con ID ${requisitoId}`)
      if (!ok) {
      ...
      try {
        await deleteRequisitoById(requisitoId)

        this.items = this.items.filter(u => u.id !== requisitoId)
