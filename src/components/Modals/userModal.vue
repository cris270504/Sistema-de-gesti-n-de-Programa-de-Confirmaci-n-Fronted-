  ...
  const draft = ref({ id: null, name: '', celular: '', dni: '', email: '', fechaNacimiento: null, roles: [] });
  ...
  async function submitUpdate() {
    ...
    const payload = { name, roles: selectedRoleNames, celular, dni, email, fecha_nacimiento: fechaNacimiento };
          ...
          <div class="modal-body">
            ...
            <form v-else @submit.prevent="submitUpdate" id="userForm" class="needs-validation">
              <div class="row g-4">
                ...
                <div class="col-12">
                  <label for="userName" class="form-label fw-bold text-secondary small text-uppercase">Nombre
                    Completo</label>
                ...
                <div class="col-md-6">
                  <label for="userEmail" class="form-label fw-bold text-secondary small text-uppercase">Correo
                    Electrónico</label>
                  <div class="input-group">
                    <span class="input-group-text bg-blue-soft text-primary border-end-0">
                ...
                <div class="col-md-6">
                  <label for="userFechaNacimiento" class="form-label fw-bold text-secondary small text-uppercase">Fecha
                    de nacimiento</label>
                  <div class="input-group">
                    <span class="input-group-text bg-blue-soft text-primary border-end-0">
                    ...
                    <div v-if="availableRoles && availableRoles.length" class="d-flex flex-wrap gap-2">
                      <template v-for="role in availableRoles" :key="role.id">
                        ...
                        <label class="role-card d-flex align-items-center gap-2 px-3 py-2 rounded-pill transition-all"
                          :for="'role-' + role.id">
                          <i class="bi"
                            :class="draft.roles.includes(role.id) ? 'bi-check-circle-fill' : 'bi-circle'"></i>
                          <span>{{ role.name }}</span>
                        </label>
                      </template>
                    </div>
                    <div v-else class="text-muted fst-italic text-center py-2">
